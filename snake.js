class Snake {
  constructor(game, x, y, speedX, speedY, color, name, image) {
    ...
    // this.image = document.getElementById("sticky_saberhound");
    this.image = image;
  ...
  update() {
    ...
    // bounderies
    if ((this.x <= 0 && this.speedX < 0) || (this.x >= this.game.columns - 1 && this.speedX > 0) || (this.y <= this.game.topMargin && this.speedY < 0) || (this.y >= this.game.rows - 1 && this.speedY > 0)) {
      this.moving = false;
    }
    if (this.moving) {
      this.x += this.speedX;
...
class ComputerAi extends Snake {
  ...
  turn() {
    // dont turn if mowing towards food
